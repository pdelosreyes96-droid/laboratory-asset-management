...
- Current role badge (updates on login)
- **Logout** button → returns to login screen (switch roles by logging in as a different user)
...
- **"+ Add Equipment"** button (Admin only) — opens modal to create new equipment entries

### Borrowing Request Modal
- Purpose textarea (min 10 characters validation)
- Expected duration input (1-30 days)

### Add Equipment Modal (Admin Only)
- Asset tag (auto-uniqueness validated)
- Equipment name, category, location
- Initial status dropdown (Available / Under Maintenance / Damaged)
- Creates new equipment + audit log entry
...
3. **Login** with test credentials (password `12345` for all):

   | Username | Role |
   |---|---|
   | `Rona` | Administrator |
   | `Icy` | Requester / Viewer |
   | `Bevelyn` | Laboratory Staff |

4. Role auto-selects based on username
5. **Add equipment** (Admin only): click "+ Add Equipment" in Equipment Catalog
6. **Submit borrow request** as Requester: click "Request Borrow" on available items
7. **Process requests** as Admin/Staff: Approve → Release → Return
8. **Logout**: click "Logout" in navbar → returns to login screen to switch roles
